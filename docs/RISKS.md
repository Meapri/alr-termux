...
| Ubuntu 26.04 rootfs | **부팅한다. uutils coreutils 만 깨진다** ([M15](evidence/2026-08-03-m15-cmdline-2604.md) — M14 의 "부팅 안 됨" 기록은 부정확했다). `bash`·`apt`·`dpkg`·`find`·`grep`·`sed`·`awk`·`tar`·`getent` 정상, `ls`/`cat`/`echo` 등 uutils 계열 전부 실패. **원인 미규명** — uutils 는 올바른 `argv[0]` 을 받고 `/proc` 도 exec 경로 auxv 도 읽지 않으면서 로더 이름을 보고한다. 같은 rootfs 를 proot 로 돌리면 정상이므로 alr 고유다. 26.04 는 v1 대상이 아니며 `alr install` 이 설치 시점에 경고한다 |
