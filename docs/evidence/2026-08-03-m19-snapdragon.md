...
- ~~**Android 12~15** — §A6를 완전히 닫는 마지막 변수. 기기 없음.~~
  → **후기 (2026-08-03, 같은 날 늦게):** 이 항목은 측정이 아니라 **범위 결정으로 닫혔다** — Android 12~15 는 검증하지 않기로 했다([ADR 0007](../adr/0007-android-16-only.md)). 위 본문의 관측과 논증은 그대로 유효하며, 그 논증이 곧 범위를 16으로 좁힌 근거다. 증거 문서는 쓰인 시점의 사실을 남기므로 본문은 고치지 않는다.
