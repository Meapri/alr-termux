...
### 7.2 그래서 임계값을 올리지 않았다

올리면 게이트가 초록으로 바뀌지만, **그건 측정이 아니라 목표를 옮기는 것**이다. 지금 확정된 사실만 적어 둔다:

- `≤ 100 ns/op` 는 **참조 기기 #1 에서 세운 절대 임계값이고 기기 간 이식되지 않는다.**
- 참조 기기 #2 의 정직한 값은 **약 110 ns/op** 이다.
- 이 게이트가 대리하는 상위 예산(총 ≤ 1.5 ms)은 **양쪽 기기에서 큰 여유로 통과한다.**

판단이 필요한 지점이라 그대로 남긴다: (a) 임계값을 기기별로 두거나, (b) 하드 게이트를 per-op 대신 총비용으로 옮기거나, (c) 110 ns 를 반영해 올리는 것. (b)가 스펙의 의도에 가장 가깝지만, 하드 불변식을 바꾸는 것은 이 문서가 혼자 할 일이 아니다.
