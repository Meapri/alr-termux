...
> **참조 #1 의 351 exec/s 도 같은 의심을 받아야 한다.** 그 측정이 유휴 상태에서 났는지 기록이 없고, 지금 그 기기가 손에 없어 다시 잴 수 없다. 아래 표의 MediaTek 열은 **워밍업 없이 측정된 값**이다.

### 6.3 배수는 기기마다 다르다 — 인용할 때 기기를 붙인다

| | 참조 #1 (MediaTek, 워밍업 없음) | 참조 #2 (Snapdragon, 워밍업 후) |
|---|---|---|
| `node -e 0` 콜드 | 6.60× (alr 55 / proot 363 ms) | **5.49×** (alr 53 / proot 291 ms) |
| exec 처리량 | 351 exec/s (proot 135) | **707 exec/s** (proot 224) |
| `git status` 10k | 34.8× (git 빌드 3종 상이) | **25.8×** (git 동일 버전, 워밍업 후) |

두 열은 **측정 조건이 다르므로 나란히 놓고 기기 차이로 읽으면 안 된다.** 특히 exec 351 vs 707 은 대부분 워밍업 유무일 가능성이 크다 — proot 쪽도 135 → 224 로 같이 올랐다는 것이 그 방향을 가리킨다. 한쪽만 빨라졌다면 alr 이야기지만, 양쪽이 같이 올랐으면 기기 상태 이야기다.
