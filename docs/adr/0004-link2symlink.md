...
Accepted, **조건부** (설계 단계, 2026-08-02). `alr doctor` P6이 디바이스에서 `link(2)` 성공을 보고하면 이 계층 전체를 끈다.
...
**기각**: `st_nlink`가 1로 남아 dpkg의 무결성 검사가 실패하고, `git clone --local`의 디스크 절약이 사라지며(대형 저장소에서 심각), pnpm의 저장소 모델이 무너진다. 그리고 한쪽을 수정하면 다른 쪽에 반영되지 않아 **조용한 데이터 불일치**를 만든다.
